...
exports.MultiIntervalSet = void 0;
...
class MultiIntervalSet {
     ...
     * @param label
     */
    getNextMiniLabel(label) {
        if (this.map.has(label)) {
            const intervalSet = this.map.get(label) ?? node_assert_1.default.fail("interval set does not exist when it should");
    ...
    add(start, end, label) {
        ...
        if (sameIntervalSet !== undefined) {
            for (const label of sameIntervalSet.labels()) {
                const sameInterval = sameIntervalSet.interval(label) ?? node_assert_1.default.fail("an interval isn't returning when we know a label definitely exists");
        ...
        for (const [_, intervalSet] of this.map.entries()) {
            for (const label of intervalSet.labels()) {
                const interval = intervalSet.interval(label) ?? node_assert_1.default.fail("intervalSet can't get the interval despite the label existing");
                const currentStart = interval.start;
                const currentEnd = interval.end;
                if (start < currentEnd && end > currentStart) {
                    throw new intervalset_js_1.IntervalConflictError("intervals conflict in MultiIntervalSet");
                }
            }
        }
        // no overlaps, so add
        if (!this.map.has(label)) {
            this.map.set(label, (0, intervalset_js_1.makeIntervalSet)());
        }
        const nextMiniLabel = this.getNextMiniLabel(label);
        const addIntervalSet = this.map.get(label) ?? node_assert_1.default.fail("map doesn't have intervalSet for label when it should");
     ...
     */
    intervals(label) {
        const out = (0, intervalset_js_1.makeIntervalSet)();
        const intervalSet = this.map.get(label);
        if (intervalSet !== undefined) {
            const intervals = [];
            for (const label of intervalSet.labels()) {
                const interval = intervalSet.interval(label) ?? node_assert_1.default.fail("no interval returned when label should exist in the intervalSet");
            ...
            for (let i = 0; i < intervals.length; i++) {
                const interval = intervals[i] ?? node_assert_1.default.fail("an interval should exist here, but was undefined instead");
...
exports.MultiIntervalSet = MultiIntervalSet;
//# sourceMappingURL=multiintervalset.js.map