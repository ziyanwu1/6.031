 ...
 */
export class DualInterval<Label> {

    public constructor(
        private readonly firstIntervalSet: IntervalSet<Label>,
        private readonly secondIntervalSet: IntervalSet<Label>
    ) {
        this.checkRep();
    }
     ...
     * @param interval2 
     */
    private overlap(interval1: Interval, interval2: Interval): number {
    ...
    public similarity(weight: number): number {
        let totalOverlap = 0;
        const allFirstIntervals: Interval[] = [];
        const allSecondIntervals: Interval[] = [];

        // get all first intervals (for loop + .push())
        for (const firstLabel of this.firstIntervalSet.labels()) {
            const interval = this.firstIntervalSet.interval(firstLabel) ?? assert.fail("no interval found for a label that should be there");
        ...
        for (const secondLabel of this.secondIntervalSet.labels()) {
            const interval = this.secondIntervalSet.interval(secondLabel) ?? assert.fail("no interval found for a label that should be there");
