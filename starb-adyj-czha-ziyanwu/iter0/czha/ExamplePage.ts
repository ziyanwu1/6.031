...
function drawStar(canvas: HTMLCanvasElement, x: number, y:number): void{
    ...
    // draw the first star component centered on the origin (which is now (x,y))
    context.strokeStyle = 'black';
    context.lineWidth = 2;
    context.strokeRect(-STAR_SIZE/2, -STAR_SIZE/2, STAR_SIZE, STAR_SIZE);

    // fill with a random semitransparent color
    context.fillStyle = BACKGROUNDS[Math.floor(Math.random() * BACKGROUNDS.length)] ?? assert.fail();
    context.fillRect(-STAR_SIZE/2, -STAR_SIZE/2, STAR_SIZE, STAR_SIZE);

    // make rotated star component -----------
 ...
 * @param y y position of center of box
 */
function drawDot(canvas: HTMLCanvasElement, x: number, y:number): void{
    const context = canvas.getContext('2d');
    assert(context, 'unable to get canvas drawing context');
    ...
    // draw the dot centered on the origin (which is now (x,y))
    context.beginPath();
    context.strokeStyle = 'black';
    context.lineWidth = 2;
    context.ellipse(0, 0, DOT_SIZE, DOT_SIZE, 0, 0, 2*Math.PI);
    context.fill();
    context.closePath();
